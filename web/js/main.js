...
const EDIT_SVG = '<svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pencil-square" fill="currentColor" xmlns="http://www.w3.org/2000/svg">\n' +
    ...
    '</svg>';
const ADD_SVG = '<svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-plus-square" fill="currentColor" xmlns="http://www.w3.org/2000/svg">\n' +
...
document.querySelector('#modalDomainBtnCommit')
    .addEventListener('click', updateDomainInfo);
document.querySelector('#modalDomainBtnDelete')
    .addEventListener('click', deleteDomainInfo);
document.querySelector('#btnShowFullLog')
    .addEventListener('click', function() {
      LOG_MESSAGE_MODAL.show();
    });

/**
 * Fill in the 'placeholder' attribute on the configuration form with config
...
/**
 * Fills in the domain configuration table based on received domain.yaml object.
 ...
 * @return {Node} completed table row
 */
...
/**
 * When you click the edit icon/button, show the modal and populate it with
 * the correct domain information (easy editing, see what you delete).
 * @param {event} e
 */
function handleEditRow(e) {
  ...
  document.querySelector('#modalDomainName')
      .setAttribute('value', domainData.name);
  document.querySelector('#modalDomainFqdn')
      .setAttribute('value', domainData.fqdn);
  document.querySelector('#modalDomainAlert')
...
/**
 * Send an update to the server with a modified (or new) domain.
 */
function updateDomainInfo() {
  ...
  updateMessageGui(`Domain update message sent. (${domainData.fqdn})`,
      domainData);
  DOMAIN_EDIT_MODAL.hide();
}

/**
 * Send a message to the server with the domain object to remove.
 */
function deleteDomainInfo() {
  ...
  updateMessageGui(`Domain deletion message sent. (${domainData.fqdn})`,
      domainData);
  DOMAIN_EDIT_MODAL.hide();
}
 ...
 * @param {string} message Summary of event
 * @param {object} extraData (optional) Data from event
 */
