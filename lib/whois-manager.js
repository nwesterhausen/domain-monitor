...
function doWhoisUpdates() {
  fs.readFile(yamler.DOMAIN_YAML_PATH)
    ...
    .then((result) => {
      console.debug("WHOIS: Cached entries: ", result);
    })
    .catch((error) => {
      throw error;
    });
...
function updateWhoisIfNeeded(domain) {
  ...
  const updateWhoisPromise = function (resolve, reject) {
  ...
  return new Promise(updateWhoisPromise);
}
    ...
    if (matched && matched.length > 1) {
      switch (matched[1]) {
        ...
        case ">>> Last update of WHOIS database":
          simplifiedObject.whois_db_update_time = matched[2].match(
            /^[^< ]+/
          )[0];
          break;
