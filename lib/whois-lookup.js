...
function simplifyWhois(whoisdata) {
    if (typeof whoisdata !== 'string') {
        return console.error("Unable to handle whois response type", typeof whoisdata);
    }
    let whoisObject = whoisdata.split(/\n/);
    ...
    for (let i=0; i<whoisObject.length; i++) {
        let matched = whoisObject[i].match(ATTRIBUTE_REGEX);
        if (matched && matched.length > 1) {
            switch (matched[1]) {
                ...
                case '>>> Last update of WHOIS database':
                    simplifiedObject.whois_db_update_time = matched[2].match(/^[^< ]+/)[0];
                    break;
                case 'URL of the ICANN WHOIS Data Problem Reporting System':
                    // Do nothing
                    break;
                default:
                    simplifiedObject.unmatched_data.push({attribute: matched[1], value: matched[2]});
                    break;
            }
        }
    }
